    ...
    for n, (q, l) in enumerate(tqdm(loader, leave=False, disable=not verbose)):
        ...
        for xx in tqdm(range(len(q)), desc="Metrics inner loop", leave=False):
            ...
            for t in range(args.early_interaction_steps):
                ...
                # PtRmtQ = torch.einsum("bnn,nm,bmd->bnd", P[xx].transpose(-1,-2), Rm_mat.T, q_)    # sanity fail
...
if __name__ == '__main__':
    ...
    if args.use_same_model:
        cembed_model_inner = qembed_model_inner
    ...
    for i in pbar:
        ...
        for n, (q, c, l) in enumerate(inner_pbar):
            ...
            for t in range(args.early_interaction_steps):
                ...
                # PtRmtQ = torch.einsum("bnn,nm,bmd->bnd", P.transpose(-1,-2), Rm_mat.T, q)     # sanity fail
                PtRmtQ = torch.bmm(P.transpose(-1,-2), Rm_mat.T @ q)
