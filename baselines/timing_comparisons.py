...
def load_models(root, name="best", device="cpu"):
    ...
    model = torch.load(f"{root}/model{suffix}.pt", map_location=device, weights_only=False)
    scoremodel = torch.load(f"{root}/scmodel{suffix}.pt", map_location=device, weights_only=False)
    embed_model = torch.load(f"{root}/embed_model{suffix}.pt", map_location=device, weights_only=False)
    if os.path.exists(f"{root}/preembed_model{suffix}.pt"):
        preembed_model = torch.load(f"{root}/preembed_model{suffix}.pt", map_location=device, weights_only=False)
    else:
    ...
    if os.path.exists(f"{root}/aggregator{suffix}.pt"):
        aggregator = torch.load(f"{root}/aggregator{suffix}.pt", map_location=device, weights_only=False)
    else:
        aggregator = None
