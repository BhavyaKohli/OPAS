...
def trajecotry_distance_list_qc(trajqs, trajcs, distance_type="hausdorff", batch_size=50, processors=30, data_name='porto'):
    # trajectory_distance_batch(1, trajs[0: batch_size], trajs, distance_type,
    #                           data_name)
    pool = multiprocessing.Pool(processes=processors)
    # print np.shape(distance)
    batch_number = 0
    for i in range(len(trajqs)):
        if (i != 0) & (i % batch_size == 0):
            print(batch_size * batch_number, i)
            # trajectory_distance_batch(i, trajs[batch_size * batch_number:i], trajs, distance_type,
            #                   data_name)
            pool.apply_async(trajectory_distance_batch,
                             (i, trajqs[batch_size * batch_number:i], trajcs, distance_type,
                              data_name))
            batch_number += 1
    pool.close()
    pool.join()
...
def trajecotry_distance_list(trajs, distance_type="hausdorff", batch_size=50, processors=30, data_name='porto'):
    ...
    for i in range(len(trajs)):
        if (i != 0) & (i % batch_size == 0):
            print(batch_size * batch_number, i)
            trajectory_distance_batch(i, trajs[batch_size * batch_number:i], trajs, distance_type, data_name)
