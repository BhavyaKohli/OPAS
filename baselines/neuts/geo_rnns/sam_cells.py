...
class SAM_LSTMCell(RNNCellBase):
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        ...
        self.bias = bias
        self.weight_ih = Parameter(torch.Tensor(5 * hidden_size, input_size-config.dimensional))
        ...
        self.spatial_embedding = SpatialExternalMemory([grid_size[0] + 3 * config.spatial_width,
                                                        ...
                                                        grid_size[2] + 3 * config.spatial_width],
                                                       hidden_size).cuda()
    ...
    def spatial_lstm_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        ...
        input = input_a[:,:-config.dimensional]
        grid_input = input_a[:,-config.dimensional:].type(torch.LongTensor).cuda() + config.spatial_width
        ...
        if self.incell:
            ...
            self.c_d = c.data
            updates = self.sg * self.spatial_embedding.read(grid_input) + (1 - self.sg) * self.c_d
            if self.training:
                self.spatial_embedding.update(grid_input, updates)
...
class SAM_GRUCell(RNNCellBase):
    ...
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        ...
        self.hidden_size = hidden_size
        self.bias = bias
        self.weight_ih = Parameter(torch.Tensor(4 * hidden_size, input_size-config.dimensional))
        self.weight_hh = Parameter(torch.Tensor(4 * hidden_size, hidden_size))
        if bias:
            self.bias_ih = Parameter(torch.Tensor(4 * hidden_size))
            self.bias_hh = Parameter(torch.Tensor(4 * hidden_size))
    ...
    def spatial_gru_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        inputs = torch.split(input_a,config.dimensional,dim=1)
        input = inputs[0]
        grid_input = inputs[1].long().cuda() + config.spatial_width
