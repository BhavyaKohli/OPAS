...
from geo_rnns.neutraj_model import NeuSeq_Network
...
class NeuTrajTrainer(object):
    ...
    def data_prepare(self, griddatapath=config.gridxypath,
        ...
        traj_grids, useful_grids, max_len = cPickle.load(
        ...
        traj_coor, useful_grids, max_len = cPickle.load(
        ...
        grid_train_seqs, grid_test_seqs = grid_trajs[:
                                                     train_size], grid_trajs[train_size:]
        coor_train_seqs, coor_test_seqs = coor_trajs[:
                                                     train_size], coor_trajs[train_size:]

        self.grid_trajs = grid_trajs[:dataset_length]
        ...
        if config.distance_type == 'dtw' or config.distance_type == 'erp' or config.distance_type == 'cdtw':
    ...
    def batch_generator(self, train_seqs, train_distance):
        ...
        while j < len(train_seqs):
            ...
            for i in range(self.batch_size):
                ...
                if j + i not in batch_trajs_keys.keys():
                ...
                for traj_index in sampling_index_list:
                    ...
                    if traj_index not in batch_trajs_keys.keys():
                        batch_trajs_keys[j + i] = 0
            ...
            yield (
                [np.array(anchor_input), np.array(trajs_input), np.array(
                    negative_input), np.array(batch_trajs_input)],
    ...
    def trained_model_eval(self, print_batch=10, print_test=100, save_model=True, load_model=None,
        ...
        spatial_net = NeuSeq_Network(2 * config.dimensional, self.target_size, self.grid_size,
        ...
        if load_model != None:
            ...
            acc1 = tm.test_model(self, embeddings,
                                 test_range=range(len(self.train_seqs), len(
                                     self.train_seqs) + config.test_num),
                                 similarity=True, print_batch=print_test, r10in50=True)
            return acc1

    def neutraj_train(self, print_batch=10, print_test=1000, save_model=True, load_model=None,
                      in_cell_update=True, stard_LSTM=False):

        spatial_net = NeuSeq_Network(2 * config.dimensional, self.target_size, self.grid_size, self.batch_size,
                                     self.sampling_num,
                                     stard_LSTM=stard_LSTM, incell=in_cell_update)
        ...
        if load_model != None:
            ...
            print(embeddings.shape)
            print(embeddings[0].shape)
            save_model_name = './features/embedding_{}_{}_{}'.format(embeddings.shape[0],
                                                                     config.data_type,
                                                                     config.distance_type)
            cPickle.dump(embeddings, open(save_model_name, 'wb'))

            tm.test_model(self, embeddings,
                          test_range=range(len(self.train_seqs), len(self.padded_trajs)),
                          similarity=True, print_batch=print_test, r10in50=True)
        for epoch in range(config.epochs):
            ...
            train_samples_num = int(
                len(self.padded_trajs) * config.seeds_radio)
            # int(len(self.padded_trajs) * config.seeds_radio)+ config.test_num

            test_range_random = list(np.random.choice(
                len(self.padded_trajs)-train_samples_num, config.test_num)+train_samples_num)
            # print(test_range_random)
            acc1 = tm.test_model(
                self, embeddings, test_range=test_range_random, similarity=True, print_batch=print_test)

            print(acc1)
            if save_model and (acc1[0] > best_preformence):
                save_model_name = './model/{}_{}_{}_training_{}_{}_incell{}' \
                    ...
                    '_test_{}_{}_{}_{}_{}_{}.h5'.format(acc1[0], acc1[1], acc1[2], acc1[3],
                                                              acc1[4], acc1[5])
                print(save_model_name)
                torch.save(spatial_net.state_dict(), save_model_name)
                best_preformence = acc1[0]
                if prev_file != '':
                    os.remove(prev_file)
                prev_file = save_model_name
            print(prev_file)
