...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, choices=["sharp", "sdtw", "mass", "fdtw"])
    ...
    METHODS = ["sharp", "sdtw", "mass", "fdtw"]
    
    dataset = args.dataset
    method = args.method

    config[method]['num_runs'] = 1
    other_methods = [m for m in METHODS if m!=method]
    config[method]['skip'] = 0
    for m in other_methods:
        config[m]['skip'] = 1
    
    test_dataset = DatasetTensors(dataset)
    ...
    print(f"Method: {method}, dataset: {dataset}, query-label-corpus shapes:", q_main.shape, l.shape, test_dataset_c_main.shape)

    SKIP_LIST = [args.skip]

    os.makedirs("../plots_and_figures/data/", exist_ok=True)
    with open(f"../plots_and_figures/data/memory_{method}.log", "a+") as main_logfile:
        ...
        for SKIP in SKIP_LIST:
            ...
            if config['sharp'].get('skip', 0)==0:
                ...
                map_ = run_baseline("SHARP", sharp_sdtw_div, q, test_dataset_c, sharp_num_c, sharp_num_runs, sharp_num_q, SKIP)

                current, max = tracemalloc.get_traced_memory()
                current, max = current / 1024**2, max / 1024**2
                print(f"{max:.4f}\t{map_:.4f}\t{SKIP}", file=main_logfile)
                tracemalloc.stop()
            ...
            if config['fdtw'].get('skip', 0)==0:
                ...
                map_ = run_baseline("FASTDTW", dtw_func, q, test_dataset_c, dtw_num_c, dtw_num_runs, dtw_num_q, SKIP)
            ...
            if config['sdtw'].get('skip', 0)==0:
                ...
                map_ = run_baseline("SDTW", sdtw_div, q, test_dataset_c, sdtw_num_c, sdtw_num_runs, sdtw_num_q, SKIP)
            ...
            if config['mass'].get('skip', 0)==0:
                try:
                    ...
                    map_ = run_baseline("MASS", mass_dist, q, test_dataset_c, mass_num_c, mass_num_runs, mass_num_q, SKIP)
