...
from opas.models.main import EmbedSkipBertEmbedder, LamModel4LongSeq
...
@torch.no_grad()
def compute_metrics_long(dataset, model, scoremodel, embed_model, preembed_model, image_embed_model=None, stagger=2, verbose=False, aggregator=None):
...
if __name__ == '__main__':
    ...
    train_dataset.q = train_dataset.q[...,::samp_rate_step]
    train_dataset.c = train_dataset.c[...,::samp_rate_step]
    val_dataset.q = val_dataset.q[...,::samp_rate_step]
    val_dataset.c = val_dataset.c[...,::samp_rate_step]
    test_dataset.q = test_dataset.q[...,::samp_rate_step]
    test_dataset.c = test_dataset.c[...,::samp_rate_step]

    # update params
    PARAMS.N = train_dataset.c.shape[1]            # length of corpus item
    PARAMS.M = train_dataset.q.shape[1]            # length of query
    PARAMS.samp_rate = train_dataset.q.shape[-1]   # input dimension
    ...
    if getattr(args, "bert_embedder", False):
        embed_model = EmbedSkipBertEmbedder(
            indim=d_model,
            outdim=args.xoutdim,
            nlayers=args.xnumlayers,
            nhead=args.xnhead
        ).to(DEVICE)
    else:
    ...
    for i in pbar:
        ...
        for n, (q, c, l) in enumerate(inner_pbar):
            ...
            if not args.cifar or args.lsun:
                # use only with audio datasets, image dataset "samp rate" is the image width, the last dim value, which does not have significance
                # mask-checking will work differently for these datasets, it has not been implemented here
                cmask = torch.isclose(c, torch.tensor(0.)).sum(dim=-1) != samp_rate     
                cmask = cmask.unsqueeze(-1)
            else:
                cmask = torch.ones_like(c)
    ...
    ################# Running test-set evaluations via the inference script, for image-sequence datasets trained using embeddings as the starting point, instead of the original images 
