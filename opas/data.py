...
def pad_corpus(c, nmax):
    if type(c) is np.ndarray:
        c = torch.from_numpy(c).float()

    pad_setting = [0 for _ in range(2*len(c.shape))]
    pad_setting[-1] = nmax - c.shape[0]
    m_a = F.pad(torch.ones(len(c)), (0, nmax - len(c)))
    c = F.pad(c, pad_setting)
    return c, m_a

class PairDatasetTrainRandLen(Dataset):
    def __init__(self, filepath, nmax, num_q=300, negative_exploration=800, seed=15):
        super().__init__()
        self.data = h5py.File(filepath, "r")
    ...
    def _pad_corpus(self, c):
        return pad_corpus(c, self.nmax)
    ...
    def __getitem__(self, idx):
        ...
        q, c = torch.from_numpy(q).float(), torch.from_numpy(c).float()
        c, m_a = self._pad_corpus(c)
        return q, c, m_a, label
    ...
    def get_positive_samples(self, num_samples=1):
        ...
        c_pl_len = [self._pad_corpus(self.c[l_]) for l_ in l]
        c = torch.stack([c[0] for c in c_pl_len])
        m_a = torch.stack([c[1] for c in c_pl_len])
...
class PairDatasetTestRandlen(Dataset):
    def __init__(self, filepath, nmax):
        super().__init__()
        self.data = h5py.File(filepath, "r")
        ...
        self.c, self.m_a = self._pad_corpus_full(self.c)
        self.c = self.c.numpy()

    def get_dataloader(self, batch_size, shuffle=True, **kwargs):
        return DataLoader(self, batch_size=batch_size, shuffle=shuffle, num_workers=8, **kwargs)
    
    def _pad_corpus_full(self, c):
        if type(c) is np.ndarray:
            c = torch.from_numpy(c).float()

        pad_setting = [0 for _ in range(2*len(c.shape)-2)]
        pad_setting[-1] = self.nmax - c.shape[1]
        pad_setting += [0, 0]
        m_a = torch.stack([F.pad(torch.ones(len(c_)), (0, self.nmax-len(c_))) for c_ in c])
        c = F.pad(c, pad_setting)
        return c, m_a
